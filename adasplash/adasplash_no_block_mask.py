...
def _get_output(
    ...
    for c_block in range(0, valid_nblocks):
        ...
        ## -- load v --
        acc += tl.dot(qk_act, v, input_precision="ieee")
...
def _bwd_preprocess(
    ...
    # write-back
    d_ptrs = DELTA + off_m
    tl.store(d_ptrs, delta, mask=m_mask)
...
def _bwd_kv_kernel(
    ...
    ## -- if we're over the seqlen for this batch, return --
    seqlen = tl.load(VARLEN + off_z).to(tl.int32)
    ...
    for c_block in range(start_block, valid_mblocks):
        ...
        qk_mask = (qk > t[:, None]) & q_mask[:, None]
        qk_mask &= causal_mask
        qk_mask = qk_mask.to(tl.float32)

        ## -- activation scores --
        qk_act = (qk - t[:, None]) * qk_mask
        qk_act = qk_act.to(input_dtype)

        ## -- load do --
        do = tl.load(do_ptrs, mask=q_mask[:, None], other=0)
        do = do.to(input_dtype)
        ...
        ## -- u = qk_mask for alpha = 2.0 --
        ds = qk_mask * (dp - delta[:, None])
...
def _bwd_q_kernel(
    ...
    ## -- in case there is, we want to only do it up to the seqlen --
    up_to_seqlen = tl.minimum((start_m + 1) * BLOCK_M, seqlen)
...
def ASSERT_CONTIGUOUS(*inputs, msg="Inputs not contiguous."):
    assert all(t.is_contiguous() for t in inputs), msg


def ASSERT_1D(*inputs, msg="Input should be one-dimensional."):
    assert all(t is None or t.dim() == 1 for t in inputs), msg
...
class _sparse_attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, alpha=1.5, is_causal=False, varlen=None, niter=10):
        ...
        ASSERT_1D(varlen, msg="varlen has to be one-dimensional.")
